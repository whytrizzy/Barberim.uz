    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md">
      <div className="w-full max-w-md glass-card-gold rounded-3xl p-6 space-y-5 animate-in fade-in zoom-in-95 duration-300">
        <div className="text-center space-y-2">
          <div className="w-14 h-14 rounded-2xl bg-gradient-to-tr from-gold to-gold-600 flex items-center justify-center text-gold-ink mx-auto shadow-lg ">
            <Scissors className="w-8 h-8 stroke-[2.5]" />
          </div>
          <h2 className="text-xl font-extrabold text-white">{t('onboardingTitle')}</h2>
          <p className="text-xs text-muted">{t('onboardingSubtitle')}</p>
        </div>
        ...
        <div className="space-y-2.5">
          ...
          <div
            ...
            className={`p-4 rounded-2xl border transition-all cursor-pointer flex items-center gap-3.5 ${
              selectedRole === 'CLIENT'
                ...
                : 'bg-surface border-line hover:border-line text-muted'
            }`}
          >
            ...
            <div className="flex-1">
              <h4 className="text-sm font-bold text-white flex items-center gap-1">
                {t('roleClient')}
                {selectedRole === 'CLIENT' && <Check className="w-4 h-4 text-gold" />}
              </h4>
              <p className="text-xs text-muted">{t('roleClientDesc')}</p>
            </div>
          </div>
          ...
          >
            ...
            <div className="flex-1">
              <h4 className="text-sm font-bold text-white flex items-center gap-1">
                {t('roleBarber')}
                {selectedRole === 'BARBER' && <Check className="w-4 h-4 text-gold" />}
              </h4>
              <p className="text-xs text-muted">{t('roleBarberDesc')}</p>
            </div>
          </div>
        </div>
